...
.inp::placeholder{color:rgba(232,244,255,.3)}.inp:focus{border-color:rgba(251,191,36,.5)}
.btn{display:flex;align-items:center;justify-content:center;width:100%;padding:15px;border-radius:14px;font-weight:800;font-size:14px;color:#fff;border:none;cursor:pointer;background:linear-gradient(135deg,#F59E0B,#D97706);font-family:inherit}
...
export default function PharmaLogin() {
  ...
  const login=async()=>{setLoading(true);setErr("");try{const r=await apiPost("/pharma/login",{email,password:pwd});setAuth(r.token,"pharma",r.user);window.location.href="/pharma/dashboard";}catch(e:any){setErr(e.message);}finally{setLoading(false);}};
  return(
    <div style={{position:"fixed",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"#020D1A",fontFamily:"'Plus Jakarta Sans',sans-serif",color:"#E8F4FF",padding:"28px 24px"}}>
      ...
      <div style={{width:"100%",maxWidth:340}}>
        <div style={{textAlign:"center",marginBottom:28}}><div style={{fontSize:40,marginBottom:12}}>💊</div><h1 style={{fontSize:22,fontWeight:900,marginBottom:4,color:"#F59E0B"}}>Pharma Login</h1><p style={{color:"rgba(232,244,255,.4)",fontSize:13}}>PMCare Pharmacy Portal</p></div>
        <div style={{marginBottom:12}}><label style={{display:"block",fontSize:10,fontWeight:700,color:"rgba(232,244,255,.4)",textTransform:"uppercase",letterSpacing:1.2,marginBottom:7}}>Email</label><input className="inp" type="email" placeholder="pharma@pmcare.org" value={email} onChange={e=>setEmail(e.target.value)} autoFocus/></div>
